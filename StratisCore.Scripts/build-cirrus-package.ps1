...
$Arg = "electron-builder build --win --x64 --config ./electron-sidechain-builder.json"
...
Set-Location $StratisCoreDir\StratisFullNode\src
Start-Process dotnet -ArgumentList "clean" -Wait
Set-Location .\Stratis.CirrusD
Start-Process dotnet -ArgumentList "publish -r win-x64 -o $StratisCoreDir\StratisCore.UI\daemon" -Wait
